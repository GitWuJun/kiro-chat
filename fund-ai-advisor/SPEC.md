...
### Phase 2.5（可视化配置面板，Streamlit）

#### 目标

提供一个本地 Web 页面，让用户无需手动编辑 JSON/YAML 即可管理持仓、调整规则、查看决策结果。

#### 技术选型

- **Streamlit**（单个 `.py` 文件即可，Python 生态内，不需要学前端）
- 入口命令：`fund-advisor ui` → 启动 `streamlit run src/fund_advisor/ui/app.py`
- 端口：默认 `localhost:8501`

#### 页面结构（4 个 Tab）

| Tab | 功能 | 数据源 | 操作 |
|---|---|---|---|
| **持仓管理** | 展示所有基金 + lots，可增删改 | `holdings.json` | 表格编辑 → 保存 |
| **规则配置** | 展示硬/软规则，调整阈值、开关 | `rules.yaml` | 滑块/开关 → 保存 |
| **目标仓位** | 饼图展示当前 vs 目标配置比例 | `targets.yaml` + `holdings.json` | 数字输入 → 保存 |
| **决策面板** | 一键跑 brief，展示结果 + 规则触发 | 调用 `api.generate_decision_brief()` | 按钮触发 |
...
│  [+ 添加基金]                                                    │
│                                                                  │
│  基金代码    名称              A/C   持仓金额    盈亏率   操作    │
│  005827     易方达蓝筹精选A     A    ¥8,050     +1.9%   [展开]  │
│  003095     中欧医疗健康A       A    ¥15,200    +32%    [展开]  │
│                                                                  │
│  ▼ 005827 展开详情                                               │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │  确认日       金额      份额       持有天数  费率   操作    │ │
│  │  2026-05-10  ¥5,000   1234.56     11天    0.5%   [删除]  │ │
│  │  2026-05-19  ¥3,000    740.12      2天    1.5%⚠️ [删除]  │ │
│  │  [+ 添加一笔]                                              │ │
│  └────────────────────────────────────────────────────────────┘ │
│                                                                  │
│  可用现金：¥50,000  [修改]                                       │
...
功能要点：
- `st.data_editor` 做可编辑表格
- 持有天数自动计算（今天 - confirm_date）
- 当前费率区间自动匹配并高亮（<7 天红色⚠️）
- 保存按钮 → 写回 `holdings.json`
...
功能要点：
- 硬规则用 `st.number_input`
- 软规则用 `st.toggle` + `st.slider`
- 保存 → 写回 `config/rules.yaml`
...
功能要点：
- `st.plotly_chart` 做饼图对比
- 偏离超过容忍带的标黄/标红
- 保存 → 写回 `config/targets.yaml`

#### Tab 4：决策面板

```
┌─────────────────────────────────────────────────────────────────┐
│  [🔄 获取今日决策 Brief]         距离 15:00 还有 28 分钟         │
│                                                                  │
│  ┌── 规则触发 ──────────────────────────────────────────────┐   │
│  │  🚫 H1 BLOCKER: 005827 有份额仅持有2天，禁止赎回         │   │
│  │  ℹ️  S3 INFO: 003095 盈利 32%，触发止盈提示              │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌── 市场快照 ──────────────────────────────────────────────┐   │
│  │  沪深300: +0.42%  创业板: -0.78%  北向: -21亿            │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌── 持仓建议矩阵 ─────────────────────────────────────────┐   │
│  │  基金           建议       依据              费用提示     │   │
│  │  005827 蓝筹   保持       H1 blocker        再等5天省¥25 │   │
│  │  003095 医疗   减仓20%   S3止盈+估值+0.5%  费用¥15.2    │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌── 相关新闻 ──────────────────────────────────────────────┐   │
│  │  14:20 财联社 | 宁德时代明日发布一季报 → 关联005827       │   │
│  │  13:45 东财   | 医药板块午后回落...     → 关联003095       │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  数据时效：估值 14:30（盘中）| 净值 T-1（昨收）               │
│  ⚠️ 本输出非投资建议，最终决策由用户负责                        │
└─────────────────────────────────────────────────────────────────┘
```

功能要点：
- 点击按钮 → 调 `api.generate_decision_brief()`
- blocker 规则红色高亮、置顶
- 建议矩阵用 `st.dataframe` + 条件格式
- 新闻按关联度排序，标注关联的基金
...
└── ui/
    ...
    ├── tab_holdings.py     # Tab 1
    ├── tab_rules.py        # Tab 2
    ├── tab_targets.py      # Tab 3
...
```bash
fund-advisor ui              # 等价于 streamlit run src/fund_advisor/ui/app.py
fund-advisor ui --port 8080  # 自定义端口
```
...
安装方式：`uv pip install -e ".[ui]"`

#### 验收标准

- [ ] `fund-advisor ui` 启动后浏览器自动打开
- [ ] Tab 1 可添加/删除基金和 lot，保存后 `holdings.json` 正确更新
- [ ] Tab 2 调整规则阈值后保存，`rules.yaml` 正确更新
- [ ] Tab 3 饼图正确展示当前 vs 目标配比
- [ ] Tab 4 点击按钮后 10 秒内展示完整决策面板
- [ ] 所有 Tab 的修改互不干扰，保存后其他 Tab 刷新即可看到最新数据
